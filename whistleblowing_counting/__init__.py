...
class Player(BasePlayer):
    ...
    counting_estimation = models.IntegerField(min=0, max=100, payoff_ecu=models.FloatField())
    ...
    def set_txt_final(self):
        ...
        txt_final = trans(dict(
            en=f"You found the right number of 1's in {self.counting_performance} "
               f"grid{pluriel(self.counting_performance)}.",
            fr=f"Vous avez trouvé le bon nombre de 1 dans {self.counting_performance} "
               f"grille{pluriel(self.counting_performance)}.",
            vi=f"Bạn đã tìm đúng số 1 trong {self.counting_performance} "
               f"lưới{pluriel(self.counting_performance)}."
        ), lang)
        ...
        if treatment == Config.INDIVIDUAL:
            txt_final += trans(dict(
                ...
                fr=f"Votre gain est donc égal à "
                   f"{self.counting_performance} x {Config.PIECE_RATE} = {self.payoff_ecu} ECU.",
                vi=f"Khoản thanh toán của bạn do đó bằng "
                   f"{self.counting_performance} x {Config.PIECE_RATE} = {self.payoff_ecu} ECU."
            ), lang)
            ...
            txt_final += trans(dict(
                ...
                fr=f"Le membre de votre groupe avec le meilleur score a trouvé le bon nombre de 1 dans "
                   ...
                   f"{Config.PIECE_RATE} = {self.payoff_ecu} ECU.",
                vi=f"Người chơi có điểm cao nhất trong nhóm của bạn đã tìm đúng số 1 trong "
                   f"{self.group.counting_performance_group} lưới{pluriel(self.group.counting_performance_group)}. "
                   f"Khoản thanh toán của mỗi thành viên trong nhóm của bạn do đó bằng "
                   f"{self.group.counting_performance_group} x "
                   f"{Config.PIECE_RATE} = {self.payoff_ecu} ECU."
            ), lang)
    ...
    def vars_for_template(player: Player):
        ...
        return dict(
            ...
            instructions_template_title=trans(dict(
                en="Task 1 - Instructions", fr="Tâche 1 - Instructions", vi="Nhiệm vụ 1 - Hướng dẫn"), lang),
...
class CountingEstimation(MyPage):
    ...
    def vars_for_template(player: Player):
        existing = MyPage.vars_for_template(player)
        counting_estimation_label = trans(dict(
            en="Your guess:", fr="Votre estimation :", vi="Dự đoán của bạn:"), player.session.vars["lang"])
        existing.update(counting_estimation_label=counting_estimation_label)
        return existing
